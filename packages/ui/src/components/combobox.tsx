	...
	return (
		<div className="text-foreground relative w-full not-has-[>*.w-full]:w-fit has-disabled:opacity-64">
			...
			<ComboboxPrimitive.Input
				...
				render={<Input className={cn("has-disabled:opacity-100", inputClassName)} nativeInput size={sizeValue} />}
				{...props}
			/>
			{showTrigger && (
				<ComboboxTrigger
	...
	return (
		<ComboboxPrimitive.Portal>
			...
			>
				<span
					className={cn(
						"bg-popover relative flex max-h-full max-w-(--available-width) min-w-(--anchor-width) origin-(--transform-origin) rounded-lg border shadow-lg/5 not-dark:bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
						className,
					)}
				>
					<ComboboxPrimitive.Popup
						className="text-foreground flex max-h-[min(var(--available-height),23rem)] flex-1 origin-(--transform-origin) flex-col transition-[scale,opacity] data-ending-style:scale-98 data-ending-style:opacity-0 data-starting-style:scale-98 data-starting-style:opacity-0"
